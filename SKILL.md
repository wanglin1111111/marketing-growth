---
name: marketing-growth
version: 1.0.0
author: wanglin1111111
description: |
  Marketing Growth 营销增长技能集合，包含 B2B 营销 playbook、开发者营销 playbook 等技能。提供 LinkedIn 内容策略、冷邮件序列、网络研讨会漏斗、ABM 定向框架、开发者关系(DevRel)计划、文档即营销、API 开发者体验、社区建设、黑客松策略等完整营销解决方案。
---

# Marketing Growth

> Merged from 19 skills

## Skill: b2b-marketing-playbook

---
name: marketing-growth
description: |
  Complete B2B marketing pipeline combining LinkedIn content, cold email sequences, and webinar funnels. Designed for SaaS founders doing $0–$1M ARR who need predictable lead generation.
---

# B2B Marketing Playbook — LinkedIn + Cold Email + Webinar Pipeline

> 🌍 **Language / 语言**: [中文](#中文版) | [English](references/en/README.md) | [日本語](references/ja/README.md) | [한국어](references/ko/README.md)

Three proven channels woven into one repeatable pipeline for B2B SaaS.

- **LinkedIn engine**: Content cadence, engagement pods, and DM-to-demo flow
- **Cold email sequences**: Subject lines, personalization, and follow-up timing
- **Webinar funnel**: Topic selection, promotion, and post-webinar nurture
- **ABM layer**: Account-based targeting for enterprise deals
- **Metrics dashboard**: Pipeline velocity, SQL rate, and CAC benchmarks

## Related Gingiris Skills
- Full version: https://clawhub.ai/skill/gingiris-b2b-growth
- All skills: https://clawhub.ai/user/gingiris
- Follow: [@WeiYipei on X](https://x.com/WeiYipei)

---

## Skill: developer-marketing-playbook

---
name: developer-marketing-playbook
description: |
  Complete developer marketing playbook covering DevRel programs, documentation as marketing, API developer experience, community building, and hackathon strategy. For dev-tool founders who need to reach engineers.
---

# Developer Marketing Playbook — DevRel, Docs & Community Funnels

> 🌍 **Language / 语言**: [中文](#中文版) | [English](references/en/README.md) | [日本語](references/ja/README.md) | [한국어](references/ko/README.md)

Developer marketing requires a different playbook than traditional B2B.

- **DevRel programs**: Building relationships with developer communities
- **Documentation as marketing**: Writing docs that convert
- **API developer experience**: DX that developers love
- **Community building**: From Discord to events
- **Hackathon strategy**: Sponsorships, prizes, and follow-up

## Related Gingiris Skills
- Full version: https://clawhub.ai/skill/gingiris-devrel
- All skills: https://clawhub.ai/user/gingiris

---

## 许可证

MIT License
